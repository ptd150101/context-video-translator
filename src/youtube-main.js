(() => {
  ...
  function extractMetadata(url, body) {
    ...
    const originalLang = u.searchParams.get('lang') || '';
    const translatedLang = u.searchParams.get('tlang') || '';
    const selectedLang = translatedLang || originalLang;

    return {
      url: String(url),
      videoId: u.searchParams.get('v') || new URL(location.href).searchParams.get('v') || '',
      // The body belongs to the track currently selected in YouTube settings.
      // Normalize tlang into rawLang so the content script does not replace an
      // auto-translated Japanese/Korean track with the video's original track.
  ...
  function postTimedText(url, body) {
    ...
    // Do not lock the first track. YouTube requests another timedtext URL when
    // the viewer changes captions, and the extension must follow that choice.
    window.postMessage({
      source: SOURCE,
